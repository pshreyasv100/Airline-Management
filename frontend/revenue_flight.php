...
$sql="SELECT flight_no,date, sum(amount) as m, count(ticket.ticket_id) as n 
from ticket, booked_leg 
where ticket.ticket_id=booked_leg.ticket_id 
group by amount,date,flight_no";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>Flight Number</th><th>Date</th><th>Sum</th><th>Amount</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["flight_no"]."</td><td>".$row["date"]."</td><td>".$row["m"]."</td><td>".$row["n"]."</td></tr>";
    }
    echo "</table>";
} 
else {	
