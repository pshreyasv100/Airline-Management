...
$sql="SELECT airport.name from airport where code in (select departure_airport_code from flight_leg where departure_airport_code in (select code from airport where country='India') and arrival_airport_code not in (select code from airport where country='$ip_country'))";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>International Airports</th></tr>';
