...
$sql="SELECT flight_number from flight_leg where departure_airport_code in (select code from airport where country='India')
and arrival_airport_code not in (select code from airport where country='$ip_country')";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>Flight Number</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["flight_number"]."</td></tr>";
    }
    echo "</table>";
} 
else {	
