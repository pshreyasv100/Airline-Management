...
$sql="SELECT avg(amount) as y from ticket
where ticket_id in 
	(select ticket_id from booked_leg 
     where flight_no in 
     	(select flight_number from flight_leg 
...
if ($result->num_rows > 0) {
    echo '<table><tr><th>Average Cost of a ticket from '.$ip_city.'</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["y"]."</td></tr>";
    }
    echo "</table>";
} 
else {	
