...
$sql="SELECT * from can_land,airport where can_land.airport_code=airport.Code";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>airport code</th><th>airport name</th><th>airplane type</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["airport_code"]."</td><td>".$row["Name"]."</td><td>".$row["airplane_type"]."</td></tr>";
    }
    echo "</table>";
} 
else {
