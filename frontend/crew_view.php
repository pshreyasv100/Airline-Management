...
echo "<h1>List of crews of different airplanes</h1>";
$sql="SELECT * from crew";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>SSN</th><th>Name</th><th>Gender</th><th>Job</th><th>airplane id</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["SSN"]."</td><td>".$row["Name"]."</td><td>".$row["Gender"]."</td><td>".$row["Job"]."</td><td>".$row["airplane_id"]."</td></tr>";
    }
    echo "</table>";
} 
else {
