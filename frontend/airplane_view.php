...
echo "<h1>List of airplanes and their types</h1>";
$sql="SELECT * from airplane,airplane_type where airplane.type_name=airplane_type.type_name";
$result = $conn->query($sql);

if ($result->num_rows > 0) {
    echo '<table><tr><th>airplane_id</th><th>Airplane type</th><th>max seats</th><th>manufacturer</th></tr>';
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["id"]."</td><td>".$row["type_name"]."</td><td>".$row["max_seats"]."</td><td>".$row["company"]."</td></tr>";
    }
    echo "</table>";
} 
else {
